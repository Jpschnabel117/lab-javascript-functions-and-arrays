...
function uniquifyArray(array) {
  ...
  let uniqueArray = [];
  let check = 0;
  for (const checkword of array) {
    // if (uniqueArray.includes(checkword)) {
    //   continue;
    // }
    let uniqueArrayRepeat = false; //without using include
    for (const word of uniqueArray) {
      if (word === checkword) {
        uniqueArrayRepeat = true;
      }
    }
    if (uniqueArrayRepeat) {
      continue;
    }
...
// Iteration #8: Bonus
const matrix = [
  [1, 2, 3, 4, 5], //greatest product is 3024
  [6, 7, 8, 9, 1],
  [2, 3, 4, 5, 6],
  [7, 8, 9, 1, 0],
  [9, 6, 4, 2, 3],
